...
- 默认使用 `Command-B` 展开或收起左侧目录栏，`Command-Shift-B` 展开或收起右侧大纲；可在“设置 → 快捷键”中修改这些绑定及其他阅读快捷键，修改立即保存，支持冲突提示和恢复默认
...
当 MoDu 尚未运行时，首次 `modu` 请求会直接使用启动窗口；当 MoDu 已经运行时，每次 `modu <path>` 都会创建一个独立阅读窗口，不会覆盖当前窗口。
