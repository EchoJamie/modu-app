...
`demo/我的知识花园/` 保存官网截图使用的演示工作区，包含阅读笔记、项目文档、设计原型和代码片段。主展示文档为 `阅读笔记/让想法慢慢生长.md`，内容按提供的参考图整理。该目录不进入站点构建产物。

从仓库根目录使用当前源码构建的预览版打开工作区和主文档：

```bash
open -a "$PWD/build/MoDu Preview.app" "$PWD/website/demo/我的知识花园" "$PWD/website/demo/我的知识花园/阅读笔记/让想法慢慢生长.md"
```

应从真实墨读窗口截图，不使用参考图或 HTML 模拟界面冒充软件截图。
